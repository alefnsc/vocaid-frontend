 ...
 * 
 * Headless Clerk sign-in form with Vocaid styling.
...
export const CustomSignIn: React.FC<CustomSignInProps> = ({
  ...
  const handleSecondFactor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        setSecondFactorError('Verification failed. Please try again.');
      }
    } catch (error: any) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      // For now, redirect to Clerk's built-in reset flow
      // In production, you'd want to handle this with a custom UI
