...
import { Headphones, Users, MessageCircle, ArrowRight, Send, Brain, HandMetal, Zap, TrendingDown, Lock, FileText, HeartPulse, Wallet, type LucideIcon } from 'lucide-react'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="text-center mb-16">
            <span className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-zinc-600 bg-zinc-200 rounded-full mb-6">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <motion.div variants={itemVariants} className="order-2 lg:order-1">
              <Card className="p-6 border-zinc-200 bg-white overflow-hidden">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-semibold text-zinc-900">{t('landing.b2bEmployeeHub.chatTitle', 'HR Knowledge Hub')}</h3>
                ...
                <div className="flex gap-4 mb-6 overflow-x-auto pb-2">
                  {MOCK_HR_KNOWLEDGE_HUB.categories.map((cat) => {
                    ...
                    return (
                      <div
                        key={cat.name}
                        className="shrink-0 px-4 py-2 bg-zinc-50 rounded-lg border border-zinc-200 hover:border-purple-300 transition-colors cursor-pointer flex items-center"
                      >
                        {IconComponent && <IconComponent className="w-4 h-4 text-purple-600 mr-2" />}
                        <span className="text-sm font-medium text-zinc-700">{cat.name}</span>
                        <span className="ml-2 text-xs text-zinc-400">({cat.count})</span>
                      </div>
                    )
                  })}
                </div>
                ...
                {/* Stats Row */}
                <div className="grid grid-cols-3 gap-4 mt-6 pt-4 border-t border-zinc-200">
                  <div className="text-center">
                    <div className="text-xl font-bold text-purple-600">{MOCK_HR_KNOWLEDGE_HUB.stats.ticketsDeflected}</div>
                    <div className="text-xs text-zinc-500">{t('landing.b2bEmployeeHub.stats.deflected', 'Tickets Deflected')}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl font-bold text-zinc-900">{MOCK_HR_KNOWLEDGE_HUB.stats.avgResponseTime}</div>
                    <div className="text-xs text-zinc-500">{t('landing.b2bEmployeeHub.stats.responseTime', 'Avg. Response')}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl font-bold text-zinc-900">{MOCK_HR_KNOWLEDGE_HUB.stats.knowledgeArticles}</div>
                    <div className="text-xs text-zinc-500">{t('landing.b2bEmployeeHub.stats.articles', 'Knowledge Articles')}</div>
                  </div>
                </div>
              </Card>
            </motion.div>
            ...
            {/* Features List */}
            <motion.div variants={containerVariants} className="space-y-4 order-1 lg:order-2">
              {B2B_HR_FEATURES.map((feature) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex gap-4 p-4 bg-zinc-50 rounded-xl border border-zinc-200 hover:border-purple-200 hover:bg-white hover:shadow-md transition-all"
                  >
              ...
              <div className="pt-4">
                <Button
                  ...
                  disabled
                  className="w-full border-zinc-300 text-zinc-500 py-6 text-base font-semibold cursor-not-allowed"
                >
                  <Users className="w-4 h-4 mr-2" />
                  {t('landing.b2bEmployeeHub.cta', 'Join Waitlist')}
                  <span className="ml-2 px-2 py-0.5 text-xs bg-zinc-200 text-zinc-600 rounded-full">
                    {t('common.soon', 'Soon')}
                  </span>
                </Button>
              </div>
